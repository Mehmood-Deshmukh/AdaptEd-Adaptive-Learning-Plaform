...
function SubjectiveAnswers() {
	...
	const generateQuestion = async (currentTopic) => {
		try {
			const model = genAI.getGenerativeModel({
				model: "gemini-1.5-flash",
			});
			const prompt = `Generate a challenging but easy subjective question about ${currentTopic}. The question should be thought-provoking and encourage a detailed response.`;
					...
					{Object.entries(evaluationMetrics).map(([key, value]) => (
						<div key={key} className="mb-4">
							...
							<p className="text-xs text-gray-500 mt-1">
								{key === "accuracy" &&
									"How factually correct your answer is."}
								{key === "completeness" &&
									"How thoroughly you covered all aspects of the topic."}
								{key === "criticalThinking" &&
									"How well you analyzed and evaluated the information."}
								{key === "clarity" &&
									"How clear and well-structured your answer is."}
								{key === "overallScore" &&
									"Your combined performance across all metrics."}
							</p>
						</div>
					))}
				...
				<div className="grid grid-cols-2 md:grid-cols-5 gap-3">
					{Object.entries(evaluationMetrics).map(([key, value]) => (
						<motion.div
							...
							transition={{
								delay:
									Object.keys(evaluationMetrics).indexOf(
										key
									) * 0.1,
							}}
						...
						>
							...
							<div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
								<motion.div
									...
									className={`h-1.5 rounded-full ${getMetricColor(
										value
									)}`}
								></motion.div>
							</div>
					...
					<AnimatePresence mode="wait">
						...
						{step === 3 && (
							...
							>
								...
								>
									<RotateCcw size={16} className="mr-2" /> Try
									Another Topic
								</motion.button>
							</motion.div>
						)}

...
import { TagIcon, Calendar, Users, Share2 } from "lucide-react";
...
const CommunityPage = () => {
	...
	const fetchCommunityData = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Failed to fetch community data"
				);
			}
	...
	const handlePostVote = async (postId, type) => {
		...
		try {
			...
			const downvotes = Array.isArray(post.downvotes)
				? post.downvotes
				: [];
			...
			if (type === "upvote") {
				...
				if (downvotes.includes(user._id)) {
					updatedPost.downvotes = downvotes.filter(
						(id) => id !== user._id
					);
				}
				...
				if (upvotes.includes(user._id)) {
					updatedPost.upvotes = upvotes.filter(
						(id) => id !== user._id
					);
				}
			...
			if (!response.ok) {
				setPosts(posts);
				console.error(
					`Error ${type}ing post: Server responded with an error`
				);
			}
		} catch (e) {
			setPosts(posts);
		...
		try {
			...
			if (!isMember) {
				const response = await fetch(
					`${backendUrl}/api/user/join-community`,
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
				...
				const response = await fetch(
					`${backendUrl}/api/user/leave-community`,
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
					...
					<div className="max-w-7xl mx-auto px-6 py-8">
						<div className="bg-white p-8 rounded-xl shadow-sm mb-6 text-center">
							...
							<p className="mt-2 text-gray-500">
								The community data could not be loaded. Please

...
const Quiz = () => {
  ...
  // Hardcoded values
  const hardcodedDifficulty = "intermediate";
  const hardcodedTags = ["algorithms", "data-structures"];

  // Generate quiz based on URL query parameters
  useEffect(() => {
    const topicFromURL = searchParams.get("topic");
    
    if (topicFromURL) {
      generateQuiz(topicFromURL);
    }
  }, [searchParams]);
  ...
  const generateQuiz = async (topic) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/quiz/generate`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            difficulty: hardcodedDifficulty,
            tags: hardcodedTags,
          }),
        }
      );
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/quiz/generate`,
        {
          ...
          body: JSON.stringify({
            ...
            topic: topic,
            domain: "Computer Science",
            difficulty: selectedDifficulty ? selectedDifficulty.code : hardcodedDifficulty,
            tags: selectedTags.length > 0 ? selectedTags.map((tag) => tag.code) : hardcodedTags,
          }),
        }
      );
        ...
        <div className="w-full max-w-xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
          <div className="px-8 py-6 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Create a Computer Science quiz on your preferred topic
            </p>
          </div>
